function [state, control, runtime, out] = FinalOpt(x0,xF,N,Ts,PlantL,stateWS,controlWS,A,b)
% Accurately solve a collision avoidance problem, optimizing the warm start path

    %% algorithm parameters
    % desired safety distance
    dmin = 0.01;        % Smaller than the value in Warm Start
    ...
    STATES_INIT(:,1) = 0:1:N;
    for i = 2:7
        STATES_INIT(:,i) = interp1(stateWS(:,1),stateWS(:,i),0:1/N,'poly');
    end

    CONTROLS_INIT = zeros(N+1,27);
    CONTROLS_INIT(:,1) = 0:1:N;
    for i = 2:27
        CONTROLS_INIT(:,i) = interp1(controlWS(:,1),controlWS(:,i),0:1/N,'poly');
    end
    ...
    BEGIN_ACADO;  
        ...
        f.add( next(L) == 0.5*timeScale + 1*timeScale^2 +...
            [x-xF(1); y-xF(2); vx; vy]'*Q*[x-2; y-2; vx; vy] + [ax; ay]'*R*[ax; ay]+...
        ...
        ocp.subjectTo( -0.0 <= ax <= 1.0  );
        ocp.subjectTo( -0.0 <= ay <= 1.0  );
        ocp.subjectTo( 0.1 <= timeScale <= 2.5  );
        ...
        algo.set( 'INTEGRATOR_TYPE', 'INT_RK4');
