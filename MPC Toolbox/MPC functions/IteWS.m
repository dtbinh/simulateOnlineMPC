function [state, control, runtime, out] = IteWS(x0,xF,N,Ts,PlantL,stateWS,controlWS,A,b)
% Iteratively solve the MPC problem with different obstacle size
    ...
    BEGIN_ACADO;  
        ...
        ocp.subjectTo( -0.0 <= ax <= 0.05  );
        ocp.subjectTo( -0.0 <= ay <= 0.05  );
        ocp.subjectTo( 0.01 <= timeScale <= 2.5  );
